...
;;(letrec (map (lambda (proc list)
;;               (if (null? list)
;;                   null
;;                   (pair (proc (fst list)) (map proc (snd list)))))))
;;(letrec (filter (lambda (pred xs)
;;                  (if (null? xs)
;;                      null
;;                      (if (pred (fst xs))
;;                          (pair (fst xs) (filter pred (snd xs)))
;;                          (filter pred (snd xs)))))))
...
(struct begin-expr  (list)          #:transparent)
...
(define (expr? e)
  (match e
    [(const v) (or (number? v) (boolean? v) (symbol? v) (null? v) (procedure?))]
    [(let-expr x e1 e2)
...
(define builtins '(number? boolean? procedure? pair? + - * / = < > pair fst snd not null? eq? read display))
(define (op->builtin op)
  (match op
    ...
    ('read (builtin (lambda (x) (read))))
    ('display (builtin (lambda (x) (display x))))
    ('eq? (builtin (lambda (x) (builtin (lambda (y) (eq? x y))))))
    ))
...
(define (make-begin xs)
  (define (iter xs)
    (if (null? xs)
        null
        (cons (parse (car xs)) (iter (cdr xs)))))
  (begin-expr (iter xs)))
...
(define PROGRAM (parse
                 '(letrec (insertsort (lambda (x)
                                        (letrec (sort (lambda (xs acc)
                                                        (letrec (insert (lambda (ys v)
                                                                          (if (null? ys)
                                                                              (pair v null)
                                                                              (if (not (< (fst ys) v))
                                                                                  (pair v ys)
                                          ...
                                          (sort x null))))
                    (insertsort (pair 3 (pair 2 (pair 1 null)))))))

(define INSERTSORT (parse
                    '(letrec (insertsort (lambda (x)
                       (letrec (sort (lambda (xs acc)
                         (letrec (insert (lambda (ys v)
                               (if (null? ys)
...
(define PROGRAM2 (parse
                  '(letrec (wczytaj (lambda (x)
                            (let (a (read))
                              (if (eq? a 0)
                                  null
                                  (begin
