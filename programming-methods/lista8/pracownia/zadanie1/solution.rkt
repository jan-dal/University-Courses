...
(define (ap-func x y)
  (if (null? y)
      x
      (match x
        [(clo id body ρ) (ap-func (eval-env body (env-add ρ id (car y))) (cdr y))]
        [_ (raise (type-error 'procedure x))])))


(define (op->proc op)
  (match op
    ('apply (lambda (x y) (if (null? y) x (ap-func x y))))
    ('+ +)
    ('- -)
    ('* *)
...
(define (make-list xs)
  (if (null? xs)
      (const null)
      (pair-expr (parse (car xs)) (make-list (cdr xs)))))
