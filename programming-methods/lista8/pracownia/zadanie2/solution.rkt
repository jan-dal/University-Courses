...
(struct cons-expr (el er)         #:transparent)

(define (operator? op)
  (member op '(+ - * / and or = < > cons)))

(define (unoperator? op)
  (member op '(head tail)))
...
(define (op->proc op)
  (match op
    ...
    ('apply (lambda (x y) (foldl (lambda (a b) (app b a)) x y)))
    ('cons cons)
    ('head car)
    ('tail cdr)))
...
(define (eval-pair e n ρ)
  (match e
    [(var-expr x) (eval-pair (env-lookup ρ x) n ρ)]
    [(pair-expr el er) (if (= n 0) (cons el er)
                           (cons (eval-pair el (- n 1) ρ) (eval-pair er (- n 1) ρ)))]
    [(snd v) (cdr (eval-pair v (+ n 1) ρ))]
    [(fst v) (car (eval-pair v (+ n 1) ρ))]
    [_ e]))

(define (eval-env e ρ)
  (match e
    [(const n)    n]
    [(var-expr x) (eval-env (env-lookup ρ x) ρ)]
    [(clo id body ρ) (clo id body ρ)]
    [(fun x e)    (clo x e ρ)]
    [(app e1 e2)
     (let ((v1 (eval-env e1 ρ)))
       (match v1
         [(clo x e ρ) (eval-env e (env-add ρ x e2))]
    ...
    [(fst e)
     (let ((v (eval-pair e 0 ρ)))
       (match v
         [(pair-expr el er) (eval-env el ρ)]
         [_ (if (pair? v) (eval-env (car v) ρ) (raise (type-error 'pair v)))]))]

    [(snd e)
     (let ((v (eval-pair e 0 ρ)))
       (match v
         [(pair-expr el er) (eval-env er ρ)]
         [_ (if (pair? v) (eval-env (cdr v) ρ) (raise (type-error 'pair v)))]))]
    
    [(let-expr x e1 e2)
       (eval-env e2 (env-add ρ x e1))]))
...
(define (reserved? s)
  (or (operator? s)
      (member s '(let list if pair fst snd lambda true false quit)))) ; quit zarezerwowane dla repla
...
(define (make-list xs)
  (foldr (lambda (a b) (binop 'cons (parse a) b)) (const null) xs))

(define (make-apply e args)
  (match e
      [(fun x expr) (if (null? args)
                        e
                        (app (fun x (make-apply expr (cdr args))) (parse (car args))))]
      [_ (if (not (null? args)) (raise (parse-error "Too many arguments")) e)]))
...
(define (parse e)
  (cond
   ...
   [(and (list? e)
         ...
         (eq? 'apply (first e))
         (list? (third e))
         (make-apply (parse (second e)) (cdr (third e))))]
   [(and (list? e)
         (= 3 (length e))
         (operator? (car e)))
