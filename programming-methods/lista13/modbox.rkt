...
(define builtins '(+ - * / = < and or eq-symbol?))
...
(define (name->type op)
  (case op
    ...
    [(eq-symbol?) (function2-type (symbol-type) (symbol-type) (symbol-type))]))
...
(define (typecheck-env e env)
  (type-case Expr e
    ...
    [(letrec-expr id e1 e2)
     (let* ([arg-t (gen-var)]
            [r1 (typecheck-env e1 (env-add id arg-t env))]
            [r2 (typecheck-env e2 (env-add id (typecheck-result-tp r1) env))])
       (typecheck-result (typecheck-result-tp r2)
                         (typeeqs-append (typecheck-result-eqs r1)
                                         (typecheck-result-eqs r2))))]
    [(if-expr eb et ef)
