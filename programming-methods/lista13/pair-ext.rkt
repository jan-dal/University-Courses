...
(define-type Expr
  ...
  (letrec-expr [id : Symbol] [e1 : Expr] [e2 : Expr]))
  ...
  (cond
    ...
    [(s-exp-list? q)
     (let ([ql (s-exp->list q)])
       (cond
         [(and (= (length ql) 3)
               (equal? (s-exp->symbol (first ql)) 'letrec)
               (s-exp-list? (second ql))
               (s-exp-symbol? (first (s-exp->list (second ql)))))
          (let ([id (s-exp->symbol (first (s-exp->list (second ql))))]
                [e1 (parse (second (s-exp->list (second ql))))])
          (letrec-expr id e1 (parse (third ql))))]
         [(and (= (length ql) 2)
               (equal? (s-exp->symbol (first ql)) 'quote)
               (s-exp-symbol? (second ql)))
               (const-symbol (s-exp->symbol (second ql)))]
         [(and (= (length ql) 3)
               (equal? (first ql) `let))
...
(define menv-empty : (MEnv 'a) empty)

(define (menv-lookup x menv)
  (type-case (MEnv 'a) menv
    [empty (error 'menv-lookup (string-append "Unknownd id" (to-string x)))]
    [(cons p ps)
     (if (eq? (fst p) x)
         (unbox (snd p))
         (menv-lookup x (rest menv)))]))

(define (menv-add x v menv) (cons (pair x (box v)) menv))

(define (menv-update! x v menv)
  (type-case (MEnv 'a) menv
    [empty (error 'menv-update! (string-append "Unknownd id" (to-string x)))]
    [(cons p ps)
     (if (eq? (fst p) x)
         (set-box! (snd p) v)
         (menv-update! x v (rest menv)))]))
...
(define-type Value
  ...
  (symbol-val [v : Symbol])
  (pair-val [p : (Value * Value)])
  (builtin [f : (Value -> Value)])
  (closure [id : Symbol] [body : Expr] [env : (MEnv Value)])
  (blackhole))
...
(define (symbol-pred f)
  (lambda (x y)
    (boolean-val (f (symbol-val-v x) (symbol-val-v y)))))
  ...
  (builtin
   (lambda (x)
     (builtin
      (lambda (y)
        (f x y))))))
(define (builtin-unary f)
  (builtin (lambda (x) (f x))))

(define (name->builtin op)
  (case op
    ['fst (builtin-unary (built-sel fst))]
    ['snd (builtin-unary (built-sel snd))]
    ['pair (builtin-binary (built-pair pair))]
    ['eq-symbol? (builtin-binary (symbol-pred equal?))]
...
(define builtins '(+ - * / = < and or eq-symbol? pair fst snd))
...
(define start-menv
  (foldr (lambda (x menv) (menv-add x (name->builtin x) menv))
         menv-empty builtins))



(define (meval-env e menv)
  (type-case Expr e
    [(const-num n) (number-val n)]
    [(const-bool b) (boolean-val b)]
    [(const-symbol x) (symbol-val x)]
    [(var-expr x) (menv-lookup x menv)]
    [(let-expr x e1 e2)
     (meval-env e2 (menv-add x (meval-env e1 menv) menv))]
    [(if-expr eb et ef)
     (if (boolean-val-v (meval-env eb menv))
         (meval-env et menv)
         (meval-env ef menv))]
    [(fun x e) (closure x e menv)]
    [(app e1 e2)
     (let ([v1 (meval-env e1 menv)]
           [v2 (meval-env e2 menv)])
       (type-case Value v1
         [(builtin f) (f v2)]
         [(closure x e cenv) (meval-env e (menv-add x v2 cenv))]
         [else (error 'meval-env "Invalid applied value!")]))]
    [(letrec-expr x e1 e2)
     (let* ([menv (menv-add x (blackhole) menv)]
            [v (meval-env e1 menv)])
       (begin
       (menv-update! x v menv)
       (meval-env e2 menv)))]))
(define (eval e) (meval-env e start-menv))
...
(define (occurs a t)
  (type-case Type t
    ...
    [(symbol-type) #f]
    [(boolean-type) #f]
    [(pair-type t1 t2) (or (occurs a t1) (occurs a t2))]
...
(define (substitute s t)
  (type-case Type t
    ...
    [(pair-type t1 t2) (pair-type (substitute s t1) (substitute s t2))]
    [(function-type t1 t2)
...
; Instatiate podmienia zmienne w (type-schema-t) za świeże zmienne jeżeli są w liście (type-schema-vs)

(define (instantiate schema)
  (let ([sub (foldr (lambda (x y) (cons (pair x (gen-var)) y)) empty (type-schema-vs schema))])
    (substitute sub (type-schema-t schema))))
...
(define (function2-type t1 t2 t3)
  (type-schema empty (function-type t1 (function-type t2 t3))))
...
(define (name->type op)
  (case op
    ...
    [(eq-symbol?) (function2-type (symbol-type) (symbol-type) (boolean-type))]
    [(pair) (type-schema (list 't1 't2) (function-type (var-type 't1) (function-type (var-type 't2) (pair-type (var-type 't1) (var-type 't2)))))]
    [(fst) (type-schema (list 't1 't2) (function-type (pair-type (var-type 't1) (var-type 't2)) (var-type 't1)))]
    [(snd) (type-schema (list 't1 't2) (function-type (pair-type (var-type 't1) (var-type 't2)) (var-type 't2)))]))
...
(define (typecheck-env e env)
  (type-case Expr e
    ...
    [(const-symbol x)
     (typecheck-result (symbol-type) typeeqs-empty)]
    [(const-bool b)
     (typecheck-result (boolean-type) typeeqs-empty)]
    [(var-expr x)
     (typecheck-result (instantiate (env-lookup x env)) typeeqs-empty)]
    [(app e1 e2)
    ...
    [(fun x e)
     (let* ([arg-t (gen-var)]
            [r (typecheck-env e (env-add x (type-schema empty arg-t) env))]
            [res-t (typecheck-result-tp r)])
       (typecheck-result (function-type arg-t res-t)
                         (typecheck-result-eqs r)))]
    [(let-expr x e1 e2)
     (let* ([r1 (typecheck-env e1 env)]
            [r2 (typecheck-env e2 (env-add x (type-schema empty (typecheck-result-tp r1)) env))])
       (typecheck-result (typecheck-result-tp r2)
                         (typeeqs-append (typecheck-result-eqs r1)
                                         (typecheck-result-eqs r2))))]
    [(letrec-expr id e1 e2)
     (let* ([arg-t (gen-var)]
            [r1 (typecheck-env e1 (env-add id (type-schema empty arg-t) env))]
            [r2 (typecheck-env e2 (env-add id (type-schema empty (typecheck-result-tp r1)) env))])
       (typecheck-result (typecheck-result-tp r2)
                         (typeeqs-append (typecheck-result-eqs r1)
                                         (typecheck-result-eqs r2))))]
    [(if-expr eb et ef)
...
(define (silnia n)
  (parse `(letrec (fact (lambda (x) (if (= x 0) 1 (* x (fact (- x 1))))))
            (fact ,(number->s-exp n)))))
(define (fibonacci n)
  (parse `(letrec (fib (lambda (x y n) (if (= n 0) x (fib y (+ x y) (- n 1)))))
            (fib 0 1 ,(number->s-exp n)))))
